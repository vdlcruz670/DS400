...
```{r}
eda <- fake_news %>% select(type, title_has_excl)
eda <- eda %>% group_by_all %>% count # getting the count of each category
eda$category <- paste(eda$type, eda$title_has_excl) # creating a new column w/the two columns combined
```
...
-   Unconditional probability means that it doesn't take the other data into consideration
...
```{r}
likelihood_article_fake_has_exc <- 0.267
```
...
```{r}
prior_probability_pop_true <- 0.37
```
...
```{r}
prior_probability_region_south <- 0.24
```
...
```{r}
likelihood_south_pop <- 0.07
```

```{r}
(prior_probability_region_south * likelihood_south_pop) / prior_probability_pop_true
```

P(S\|A) = P(S)LP(S/A) / P(A)
...
```{r}
prior_prob_pop <- sum(priors*likelihoods)

prior_prob_pop
```

*This takes into account the census priors. We multiply it by the likelihoods of people who say pop, so that we can get the prior prob of people who say pop for the general US*

4\) Finding our posterior probability P(S\|A)

```{r}
(priors[3] * likelihoods[3]) / prior_prob_pop
```
